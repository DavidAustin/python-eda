...
def make_fp(x_pins, y_pins, part_w, part_h, x_pads_offset, y_pads_offset, px, py, pad_w, pad_h, ground_pad_w, ground_pad_h):
    g = FootprintGen('VQFN-%dx%d-%.2fx%.2f-%.2fx%.2f-%.2fx%.2f-%.2fx%.2f' % (x_pins, y_pins, part_w, part_h, x_pads_offset, y_pads_offset, px, py, pad_w, pad_h))
...
# However, mk_VQFN24_PAD.py has large inner pins, so maybe don't replace that one

# https://www.ti.com/lit/ds/symlink/msp430fr2433.pdf
make_fp(6, 6, 4.0, 4.0, 3.825, 3.825, 0.5, 0.5, 0.24, 0.58, 2.7, 2.7)
